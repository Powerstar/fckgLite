...
function UpdateImage( e, skipId)
{
	...
	if ( oEditor.FCKBrowserInfo.IsIE )
	{
        ...
        if(e.align == 'middle')	{
            e.className = 'mediacenter';
        }
        else if(e.align == 'right')	{
            e.className = 'mediaright';
        }
        else if(e.align == 'left')	{
            e.className = 'medialeft';
	      
        }
        else if(e.align == 'baseline') {
               e.className = 'baseline';
        
       }
        else if(e.align == 'none') {
               e.className = 'media';
               SetAttribute( e, 'align'	, '' ) ;
       }
        else {
	...
	else
	{
        ...
        if(e.align == 'middle')	{
             SetAttribute( e, 'class'	, 'mediacenter' ) ;
        }
        else if(e.align == 'right')	{
             SetAttribute( e, 'class'	, 'mediaright' ) ;
        }
       else if(e.align == 'left')	{
               SetAttribute( e, 'class'	, 'medialeft' ) ;       

       }
       else if(e.align == 'baseline') {
               SetAttribute( e, 'class'	, 'baseline' ) ;
        
       }
       else if(e.align == 'none') {
               SetAttribute( e, 'class'	, 'media' ) ;
               SetAttribute( e, 'align'	, '' ) ;
       }
       else {
...
function OnUploadCompleted( errorNumber, fileUrl, fileName, customMsg )
{
	...
	switch ( errorNumber )
	{
		...
		case 101 :	// Custom warning
			...
			break ;
		case 201 :
			alert( DlgImgAlertName + '"' + fileName + '"' ) ;
			break ;
		case 202 :
...
function CheckUpload()
{
	...
	if ( sFile.length == 0 )
	{
		alert(FCKLang.FileBrowserError_File_Submit) ;
		return false ;
	}
		...
		( FCKConfig.ImageUploadDeniedExtensions.length > 0 && oUploadDeniedExtRegex.test( sFile ) ) )
	{
     ...
     var fname = sFile;
     var result = sFile.match(/([\/\\])/);
     if(result) {          
          var tmp = fname.split(result[1]);
          fname = tmp.pop();           
     }
   
/*
     var pos = fname.search(/[A-Z\s]/);

     if(pos != -1) {
        var customMsg = "DokuWiki does not recognize image names with spaces\nor capital letters:\n"
        + "         " + sFile
        + "\nUpload using the DokuWiki mediamanager to convert file\nname to the correct form";     
     	OnUploadCompleted( 1, "", fname, customMsg );
	    return false ;
     }
 */
	// Show animation
